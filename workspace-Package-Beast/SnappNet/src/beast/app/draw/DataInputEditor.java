...
public class DataInputEditor extends InputEditor.Base {
	...
	@Override
	public Class<?> type() {
		return snappNetProject.core.SnapData.class;
	}
	...
	public void init(Input<?> input, BEASTInterface plugin, int itemNr, ExpandOption bExpand, boolean bAddButtons) {
		...
		this.itemNr = itemNr;
		System.out.println("CE Je suis avant le taxonset de DataInputEditor\n");
		System.out.println("CE voici input alignment " + ((Alignment) input.get()).getSiteCount()  + "\n");
		System.out.println("CE Voici le taxonset" + ((snappNetProject.core.SnapData) input.get()).m_taxonsets.get() + "\n");
		List<TaxonSet> taxonset = ((snappNetProject.core.SnapData) input.get()).m_taxonsets.get(); 
	...
	private void guess() {
		...
		try {
	        ...
	        case pattern:
				...
				((SnapData)m_input.get()).guessTaxonSets(m_sPattern, 0);
